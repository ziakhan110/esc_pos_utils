...
class Generator {
  ...
  int _getCharsPerLine(PosStyles styles, int? maxCharsPerLine) {
    ...
    if (maxCharsPerLine != null) {
      ...
      charsPerLine = _paperSize.fontACharsPerLine;
    }
    return charsPerLine;
  }
  ...
  List<int> reset() {
    ...
    bytes += setGlobalFont(_paperSize.fontType);
    return bytes;
  }
  ...
  List<int> setGlobalFont(PosFontType font, {int? maxCharsPerLine}) {
    List<int> bytes = [];
    _paperSize.fontType = font;
    _paperSize.fontACharsPerLine = maxCharsPerLine ?? _paperSize.fontACharsPerLine;
  ...
  List<int> setStyles(PosStyles styles, {bool isKanji = false}) {
    ...
    } else if (_paperSize.fontType != _styles.fontType) {
      bytes += _paperSize.fontType == PosFontType.fontB ? cFontB.codeUnits : cFontA.codeUnits;
      _styles = _styles.copyWith(fontType: _paperSize.fontType);
    }
  ...
  List<int> hr({String ch = '-', int? len, int linesAfter = 0}) {
    List<int> bytes = [];
    int n = len ?? _paperSize.fontACharsPerLine;
